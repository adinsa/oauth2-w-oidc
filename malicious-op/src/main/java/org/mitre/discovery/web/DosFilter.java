 ...
 * <p>
 * Takes init-param 'paths' containing single or comma-separated list of paths for which to perform the attack and is
 * only enabled when the {@value #DOS_SPRING_PROFILE_NAME} Spring profile is active.
 *
 * @author amar
 *
 */
public class DosFilter implements Filter {
    ...
    private static final String DOS_SPRING_PROFILE_NAME = "dos";
    ...
    public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain)
        ...
        if (this.paths.contains(path) && dosProfileIsActive(request.getServletContext())) {
    ...
    /**
     * Returns <code>true</code> if the {@value #DOS_SPRING_PROFILE_NAME} Spring profile is active.
     *
     * @param servletContext The {@link ServletContext}
     * @return <code>true</code> if the {@value #DOS_SPRING_PROFILE_NAME} Spring profile is active
     */
    private boolean dosProfileIsActive(final ServletContext servletContext) {
        final WebApplicationContext springContext = WebApplicationContextUtils.getWebApplicationContext(servletContext);
        return Arrays.asList(springContext.getEnvironment().getActiveProfiles()).contains(DOS_SPRING_PROFILE_NAME);
    }
